...
periodic batch should invoke::
    ...
    python -m akgentic.infra.maintenance --only vector --apply
...
runs, and a deployment running none simply reports the workspace. This CLI reads
none of the cluster variables below itself; each backend owns its own pair, and
the registry is what this module asks.

=============================  ================================================
``AKGENTIC_WEAVIATE_URL``      Provisions Weaviate. Unset, it is skipped.
``AKGENTIC_WEAVIATE_API_KEY``  Optional Weaviate credential.
``AKGENTIC_QDRANT_URL``        Provisions Qdrant. Unset, it is skipped.
...
from akgentic.infra.maintenance.vector_backends import sweepable_backends
...
app = typer.Typer(
    ...
    help="Remove vector-store rows and workspaces owned by deleted teams.",
)
...
def _build_reapers(only: set[ResourceKind]) -> list[TeamResourceReaper]:
    ...
    The vector reapers come from the registry rather than from a hardcoded
    vendor: one per registered backend this sweep can read and the environment
    has provisioned. A backend that cannot even be constructed — Qdrant without
    the ``qdrant-client`` extra, say — is reported and skipped rather than
    ...
    Returns:
        Reapers in a stable order: every configured vector backend sorted by
        name, then the workspace filesystem. The workspace reaper needs no
        configuration — a missing root simply scans empty — so it is always
        built, and the deletions it proposes are the ones an operator should
        read most carefully.
    """
    reapers: list[TeamResourceReaper] = []
    if not only or ResourceKind.VECTOR in only:
        for name in sweepable_backends():
            try:
                reapers.append(VectorStoreReaper(name))
            except Exception as exc:  # noqa: BLE001 - a missing extra is not a crash
                typer.echo(f"Vector-store reaper for '{name}' unavailable: {exc}", err=True)
