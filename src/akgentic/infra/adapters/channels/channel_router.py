        ...
        **Nothing is verified — deliberately, and it is the safer of the two
        ...
        await self._registry.register(
            ChannelBinding(
                ...
                # Same metadata ``initiate_team`` stores. Without this a chat
                # that joined a team with ``/register`` would carry none, and an
                # adapter that routes on it would silently fall back to a
                # default — for Signal, answering from the wrong bot number.
                metadata=self.address.metadata,
            )
        )
        logger.info(
