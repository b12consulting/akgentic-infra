...
class TestRestoringGuard:
    """Story 13.9 AC2: set_restoring() guard skips on_message during restore replay."""

    def test_restoring_true_skips_on_message(self) -> None:
        """AC2: on_message does NOT append to event stream when restoring=True."""
        stream = LocalEventStream()
        subscriber = EventStreamSubscriber(event_stream=stream)
        team_id = uuid.uuid4()
    ...
    def test_restoring_false_allows_on_message(self) -> None:
        """AC2: on_message appends to event stream when restoring=False."""
        stream = LocalEventStream()
        subscriber = EventStreamSubscriber(event_stream=stream)
        team_id = uuid.uuid4()
    ...
    def test_restoring_toggle_resumes_normal_operation(self) -> None:
        """AC2: toggling restoring True→False resumes normal forwarding."""
        stream = LocalEventStream()
        subscriber = EventStreamSubscriber(event_stream=stream)
        team_id = uuid.uuid4()
    ...
    def test_set_restoring_exists(self) -> None:
        """AC2: EventStreamSubscriber exposes set_restoring method."""
        subscriber = EventStreamSubscriber(event_stream=LocalEventStream())
        assert hasattr(subscriber, "set_restoring")
